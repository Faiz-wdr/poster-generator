...
export default function PublicEvent({ overrideSlug }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      const published = r.filter(item => !item.status || item.status.toLowerCase() === 'published');
      ...
      if (settings) {
        ...
        const tms = Array.isArray(settings.teams) ? settings.teams : [];

        let merged = [...pts];
        tms.forEach(tName => {
          if (!merged.find(m => m.name === tName)) {
            merged.push({ name: tName, points: 0 });
          }
        });

        // Sort by points descending
        setTeamPoints([...merged].sort((a, b) => (Number(b.points) || 0) - (Number(a.points) || 0)));
  ...
  // Show team points section when teams/points exist
        ...
        {hasAnyTeamPoints && (
          <section id="public-points-section" style={{ marginBottom: 40 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              ...
              <span className="hz-pill-badge">
                After #{teamPointsAfterResults || 0} Results
              </span>
            </div>
